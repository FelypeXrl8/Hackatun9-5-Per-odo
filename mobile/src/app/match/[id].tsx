...
export default function MatchDetails() {
  const { id, goalsA: initialGoalsA, goalsB: initialGoalsB } =
  ...
  useEffect(() => {
    if (typeof initialGoalsA === "string") {
      setGoalsA(initialGoalsA);
    }

    if (typeof initialGoalsB === "string") {
  ...
  async function handleSaveBet() {
    ...
    try {
      ...
      Alert.alert(
        "Palpite salvo",
        `Seu palpite para ${match.selecaoA.nome} x ${match.selecaoB.nome} foi ${goalsA} x ${goalsB}.`
      );
  ...
  return (
    <View style={styles.container}>
      <Text style={styles.title}>
        {initialGoalsA || initialGoalsB ? "Editar Palpite" : "Registrar Palpite"}
      </Text>
      ...
      <View style={styles.card}>
        ...
        <View style={styles.scoreArea}>
          <TextInput
            ...
            editable={match.abertaParaPalpite}
          />
        ...
        <TouchableOpacity
          style={[styles.button, (saving || !match.abertaParaPalpite) && styles.buttonDisabled]}
          onPress={handleSaveBet}
          activeOpacity={0.8}
          disabled={saving || !match.abertaParaPalpite}
        >
          {saving ? (
            <ActivityIndicator color={colors.white} />
          ) : (
            <Text style={styles.buttonText}>
              {match.abertaParaPalpite ? "Salvar Palpite" : "Palpite bloqueado"}
            </Text>
          )}
        </TouchableOpacity>
      </View>
